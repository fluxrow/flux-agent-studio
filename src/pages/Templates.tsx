...
export default function Templates() {
  ...
  const list = templates.filter(t => (cat==="Todos" || t.category===cat) && t.name.toLowerCase().includes(q.toLowerCase()));

  return (
    <div className="p-6 lg:p-8 space-y-6 max-w-[1600px] mx-auto">
      <div>
        <h1 className="font-display text-3xl font-bold">Templates</h1>
        <p className="text-muted-foreground text-sm mt-1">Biblioteca de bots prontos. Instale em 1 clique.</p>
      </div>
        ...
        <div className="flex gap-1.5 flex-wrap">
          {cats.map(c => (
            <button key={c} onClick={()=>setCat(c)}
              className={`px-3 py-1.5 text-xs rounded-full border transition ${cat===c ? "gradient-primary text-primary-foreground border-transparent shadow-elegant" : "border-border bg-card/60 text-muted-foreground hover:text-foreground"}`}>{c}</button>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {list.map((t) => (
          <div key={t.id} className="group rounded-2xl border border-border bg-card/60 overflow-hidden hover:border-primary/40 hover:shadow-elegant transition">
            <div className={`h-32 bg-gradient-to-br ${t.color} relative grid-bg`}>
              <div className="absolute inset-0 bg-background/20" />
              <div className="absolute bottom-3 left-3 text-[10px] uppercase tracking-widest bg-background/70 backdrop-blur px-2 py-0.5 rounded-full">{t.category}</div>
